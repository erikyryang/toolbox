...
- [x] 1.2 Escrever o parser de linha: número com ponto ou vírgula decimal, unidade opcional, caixa e espaço indiferentes, formas IEC (`KiB`, `MiB`, `GiB`, `TiB`) como sinônimo do degrau
...
- [x] 1.4 Montar o motor: uma linha de saída por linha de entrada, linha em branco preservada, e a nota que declara base aplicada e nome IEC do degrau
- [x] 1.5 Erros como `OperationError` com mensagem legível, citando a linha problemática; entrada vazia devolve saída vazia
...
- [x] 2.3 Cobrir o sufixo na entrada vencendo a seleção, e as formas IEC
- [x] 2.4 Cobrir multilinha, linha em branco preservada e origem igual ao destino
- [x] 2.5 Cobrir a formatação: inteiro sem casas, `0.1 GB → MB` sem cauda binária, valor grande sem notação exponencial
- [x] 2.6 Cobrir os erros: texto no lugar do número, unidade desconhecida, entrada vazia sem erro

## 3. Catálogo e navegação

- [x] 3.1 Acrescentar `Unidades` ao tipo `OperationGroup` em `web/src/lib/operations/types.ts`
- [x] 3.2 Criar `web/src/lib/operations/units-catalog.ts` com a operação `tamanho-de-dados`: rótulos dos dois sentidos, opções de origem, destino e base, placeholder e aliases
...
- [x] 6.1 `npm run lint`, `npm run typecheck` e `npm test` em `web/`
- [x] 6.2 `npm run build` — confirmar que a rota `/tamanho-de-dados` é gerada estaticamente
- [x] 6.3 Conferir a tela no navegador nos dois idiomas: seção nova na sidebar, conversão em tempo real, nota de base e inversão de sentido
- [x] 6.4 `openspec validate add-data-size-conversion --strict`
