...
export const en = {
  ...
  "error.browserMemory": "The browser could not reserve memory for {size}. Local processing loads the whole file into the tab's memory, and browsers cap that at a few GB regardless of how much RAM the machine has. For a file this size, use the Python script below: it does the same job on your machine, with no limit.",
  "error.entryTooLarge": "{format} cannot hold {name} ({size}): the format caps each entry at {limit}. Pick another format.",
...
export const pt: Record<MessageKey, string> = {
  ...
  "error.browserMemory": "O navegador não conseguiu reservar memória para {size}. O processamento local carrega o arquivo inteiro na memória da aba, e os navegadores limitam isso a poucos GB, independente da RAM da máquina. Para um arquivo desse tamanho, use o script Python abaixo: ele faz a mesma coisa na sua máquina, sem limite.",
  "error.entryTooLarge": "{format} não comporta {name} ({size}): o formato limita cada entrada a {limit}. Escolha outro formato.",
